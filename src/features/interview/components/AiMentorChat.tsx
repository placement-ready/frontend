...
import { mentorApi } from '@/features/interview/api';
...
const AiMentorChat: React.FC = () => {
  ...
  const handleSendMessage = async (text?: string, category?: Message['category']) => {
    ...
    try {
      const response = await mentorApi.askQuestion(messageText);
      const mentorReply =
        typeof response === 'string'
          ? response
          : ((response as { data?: string; message?: string })?.data ??
            (response as { data?: string; message?: string })?.message ??
            'Here is some guidance based on your question.');
      setMessages((prev) => [
        ...prev,
        {
  ...
  return (
    <div className="min-h-screen bg-green-50 flex flex-col max-w-4xl mx-auto relative px-4 sm:px-6">
      ...
      <div className="absolute left-0 right-0 bottom-0 px-4 sm:px-6 py-3 bg-white dark:bg-gray-800 border-t border-green-100 dark:border-green-700 flex items-center gap-2 sm:gap-3">
        ...
        />
        <button
          className="rounded-lg bg-green-500 text-white p-2 sm:p-3 font-bold shadow hover:bg-green-600 transition disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
